...
def id3(data_obj: object, attributes: dict, labels: list, depth=-1, maxDepth=-1):
    label = labels[0]
    same_label = True
    ...
    attribute_name = get_best_attribute(data_obj, attributes)
    ...
    if attribute_name:
        ...
        for v in data_obj.get_attribute_possible_vals(attribute_name):
            ...
            if len(new_data_obj) == 0:
                ...
                root.addChild(v, id3(new_data_obj, new_attributes, new_data_obj.get_column('label'), depth + 1, maxDepth=maxDepth))
    return root
...
def get_best_attribute(data_obj, attributes):
    max_gain = ('', 0.0)
...
def predict(data_obj, example, id3_tree_root):
    ...
    return root.get_attribute_name(), pred