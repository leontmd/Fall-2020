		...
		for (int j = 0; j < blockSize; j++) {
			...
			for (int k = 0; k < blockSize; k++) {
				for (int l = 0; l < blockSize; l++) {
					...
					C[i * 8 + j] += u * v * A[k * 8 + l] * std::cos(((2*k+1)*i*M_PI)/(2*blockSize)) * std::cos(((2*l+1)*j*M_PI)/(2*blockSize));
				}
			}
		...
		for (int j = 0; j < blockSize; j++) {
			...
			for (int k = 0; k < blockSize; k++) {
				for (int l = 0; l < blockSize; l++) {
					...
					B[i * 8 + j] += u * v * C[k * 8 + l] * std::cos(((2 * i + 1)*k*M_PI) / (2 * blockSize)) * std::cos(((2 * j + 1)*l*M_PI) /( 2 * blockSize));
				}
			}
