	...
	)
{
	...
	if (L.dot(N) > 0) resColor = L.dot(N)*0.333*kd*diffuseColor;
	return resColor;
}
               ...
               const float alpha) // shininess constant
{
	...
	Vector3f vec = 2 * N*N.dot(L) - L;
	float f;
	if (vec.dot(V) > 0) {
		f = std::pow(vec.dot(V), alpha);
	}
	else {
		f = std::pow(0, alpha);
	}

	resColor = diffuse(L, N, diffuseColor, kd) + 0.333*specularColor*ks*f;
	...
	const std::vector<Sphere> &spheres)
{
	...
	for (unsigned int s = 0; s < spheres.size(); s++) {
		...
		if (doesIntersect) {
			if (t0 > ct0) {
				t0 = ct0;
				sphere = s;
				point = rayOrigin + (t0 * rayDirection);
			}
		}
	...
	for (unsigned int l = 0; l < lightPositions.size(); l++) {
		Vector3f light_vec = lightPositions[l] - point;
		...
		for (unsigned int s = 0; s < spheres.size(); s++) {
			...
			if (spheres[s].intersect(point, light_vec, l0, l1)) {
				doesIntersect = true;
			}
		}
		if (!doesIntersect) {
			...
			Vector3f sur_norm = point - spheres[sphere].center;
			sur_norm.normalize();

			// This is for the diffuse 
			//B += diffuse(lightVector, surfaceNorm, spheres[closestSphere].surfaceColor, 1);

			Vector3f ray_norm = rayDirection * -1;
			Vector3f specular_color = Vector3f::Ones();

			// This is for phong shading
			B += phong(light_vec, sur_norm, ray_norm, spheres[sphere].surfaceColor, specular_color, 1, 3, 100);
		}
	}
