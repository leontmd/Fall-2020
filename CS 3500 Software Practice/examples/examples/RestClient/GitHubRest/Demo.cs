...
namespace Restful
{
    public class Demo
    {
        ...
        // You'll need to put an organization name here
        private const string ORG = "UofU-CS3500-S18";
        ...
        public static void GetWithParamsDemo()
        {
            using (HttpClient client = CreateClient())
            {
                ...
                String url = String.Format("/repos/{0}/{1}/commits?sha={2}", ORG, "spreadsheet", Uri.EscapeDataString("PS2"));
        ...
        public static void PutDemo()
        {
            using (HttpClient client = CreateClient())
            {
                ...
                String url = String.Format("/repos/{0}/{1}/contents/{2}", USER_NAME, "DemoRepo", "file1");
                StringContent content = new StringContent(JsonConvert.SerializeObject(data), Encoding.UTF8, "application/json");
        ...
        public static async void DeleteDemo()
        {
            using (HttpClient client = CreateClient())
            {
                ...
                String url = String.Format("/repos/{0}/DemoRepo", USER_NAME);
