...
namespace LMS.Controllers
{
  ...
  public class AccountController : CommonController
  {
    ...
    {
      // NOTE: All the commented code makes no difference, except the while loop will start
      // a chain of exceptions that are caught because accessing the same Students object in insert

      //var transaction = db.Database.GetDbConnection().BeginTransaction(System.Data.IsolationLevel.Serializable);
      ...
      //db.Database.AutoTransactionsEnabled = false;
      // Get largest uid from all students, professors, and admins
      // and increment by 1
      uid = GetNextUid();

      //using (var transaction = db.Database.GetDbConnection().BeginTransaction(System.Data.IsolationLevel.Serializable))
      //{
      //  db.Database.UseTransaction(transaction);

      // TODO: Allow simultaneous registrations
      //System.Threading.Thread.Sleep(5000);

      success = TryInsertUser(uid, fName, lName, DOB, SubjectAbbrev, role);
    ...
    private string GetNextUid()
    {
      ...
      {
        if (int.Parse(q.uid) >= max)
        {
          max = int.Parse(q.uid) + 1;
        }
      }
