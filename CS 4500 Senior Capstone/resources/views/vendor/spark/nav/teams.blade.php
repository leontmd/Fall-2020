...
@if (Spark::createsAdditionalTeams())
    <a class="dropdown-item" href="/settings#/{{Spark::teamsPrefix()}}">
        <i class="fas fa-fw text-left fa-btn fa-plus-circle"></i> {{__('teams.create_team')}}
    </a>
@endif
...
@if (Spark::showsTeamSwitcher())
    <a class="dropdown-item" v-for="team in teams" :href="'/settings/{{ Spark::teamsPrefix() }}/'+ team.id +'/switch'">
        <span v-if="user.current_team_id == team.id">
            <i class="fas fa-fw text-left fa-btn fa-check text-success"></i> @{{ team.name }}
        </span>

        <span v-else>
            <img :src="team.photo_url" class="spark-profile-photo-xs" alt="{{__('Team Photo')}}" /><i class="fas fa-btn"></i> @{{ team.name }}
        </span>
    </a>
@endif
